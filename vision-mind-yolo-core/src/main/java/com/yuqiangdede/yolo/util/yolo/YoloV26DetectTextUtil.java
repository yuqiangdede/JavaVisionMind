...
@Slf4j
public class YoloV26DetectTextUtil extends YoloBaseUtil {

    private static volatile Model yoloTextModel;

    private static Model getYoloTextModel() {
        if (yoloTextModel == null) {
            synchronized (YoloV26DetectTextUtil.class) {
                if (yoloTextModel == null) {
                    try {
                        yoloTextModel = load(Constant.YOLO_TEXT_ONNX_PATH, Constant.YOLO_TEXT_NMS_ENABLED);
        ...
        return yoloTextModel;
    }

    public static YoloDetectionResult predictor(Mat mat, Float conf) {
        return predictor(mat, getYoloTextModel(), conf);
    }
